...
根据Apache License 2.0（以下简称“许可证”）许可；除非符合许可证规定，否则不得使用此文件。
您可以在以下网址获取许可证副本：
...
/**
 * 此类封装了一些AspectJ内部知识，这些知识应该在未来版本中推回到AspectJ项目中。
 *
 * <p>它依赖于AspectJ的特定实现知识来破坏封装并执行AspectJ未设计到的操作：查询将要执行的运行时测试的类型。这里的代码应迁移到`ShadowMatch.getVariablesInvolvedInRuntimeTest()`或类似的操作。
 *
...
class RuntimeTestWalker {
    ...
    /**
     * 如果测试使用了this、target、at_this、at_target和at_annotation这些变量中的任何一个，
     * 则它测试的是子类型敏感的变量。
     */
    ...
    private abstract static class InstanceOfResidueTestVisitor extends TestVisitorAdapter {
        ...
        public void visit(Instanceof i) {
            ...
            try {
                // 不要使用ResolvedType.isAssignableFrom()，因为它不会知道（Spring）混入（mixins）。
                if (typeClass == null) {
    ...
    /**
     * 检查目标（类型）的残基种类。有关更多详细信息，请参阅SPR-3783。
     */
    ...
    /**
     * 检查是否为这种(TYPE)类型的残基。有关更多详细信息，请参阅SPR-2979。
     */
    private static class ThisInstanceOfResidueTestVisitor extends InstanceOfResidueTestVisitor {
        ...
        // TODO：优化：仅当this()指定了一个类型而不是一个标识符时才处理。
    ...
    private static class SubtypeSensitiveVarTypeTestVisitor extends TestVisitorAdapter {
        ...
        public void visit(HasAnnotation hasAnn) {
            // 如果你之前认为事情已经糟糕透了，现在我们更是跌落到恐怖的新低点...
