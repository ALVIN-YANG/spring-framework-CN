...
/**
 * 内部代码生成器，用于创建一个 {@link InstanceSupplier}，通常以
 * {@link BeanInstanceSupplier} 的形式出现，保留用于实例化 bean 的可执行程序。
 *
 * <p>生成的代码通常是一个方法引用，用于生成 {@link BeanInstanceSupplier}，
 * 但也可以使用一些快捷方式，例如：
 * <pre class="code">
...
class InstanceSupplierCodeGenerator {
    ...
    private static final javax.lang.model.element.Modifier[] PRIVATE_STATIC = { javax.lang.model.element.Modifier.PRIVATE, javax.lang.model.element.Modifier.STATIC };
    ...
    private CodeBlock generateCodeForAccessibleConstructor(String beanName, Class<?> beanClass, Constructor<?> constructor, boolean dependsOnBean, Class<?> declaringClass) {
        ...
        GeneratedMethod generatedMethod = generateGetInstanceSupplierMethod(method -> buildGetInstanceMethodForConstructor(method, beanName, beanClass, constructor, declaringClass, dependsOnBean, PRIVATE_STATIC));
    ...
    private CodeBlock generateCodeForAccessibleFactoryMethod(String beanName, Method factoryMethod, Class<?> declaringClass, boolean dependsOnBean) {
        ...
        GeneratedMethod getInstanceMethod = generateGetInstanceSupplierMethod(method -> buildGetInstanceMethodForFactoryMethod(method, beanName, factoryMethod, declaringClass, dependsOnBean, PRIVATE_STATIC));
    ...
    private CodeBlock generateReturnStatement(GeneratedMethod generatedMethod) {
        return generatedMethod.toMethodReference().toInvokeCodeBlock(ArgumentCodeGenerator.none(), this.className);
    }
